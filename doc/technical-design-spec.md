...
| ADR-14 | SSO handoff codes are persisted in the database as SHA-256 hashes, not kept in `IMemoryCache` | A handoff code is a one-time bearer credential for token issuance, not a low-risk cached permission read. The exchange can land on a different API instance behind a load balancer, so process-local state would make SSO fail intermittently in a horizontally scaled deployment. Relational consumption is an atomic conditional update (with the provider-specific equivalent where a provider lacks `UPDATE ... RETURNING`) so replay races cannot issue tokens twice | `IMemoryCache` — rejected: instance-local state and restart loss are unacceptable for a credential exchange; read-then-write — rejected: vulnerable to concurrent replay |
| ADR-15 | OIDC uses the first-party `Microsoft.AspNetCore.Authentication.OpenIdConnect` 10.0.11 package as an optional, named scheme with a dedicated cookie correlation scheme | Package 10.0.11 was verified through restore as the current .NET 10 package used by this solution and targets `net10.0`. Keeping OIDC named and non-default preserves Bearer as the API's default authentication scheme; `.AddCookie()` supplies the correlation/nonce state required by the redirect handshake. `RequireHttpsMetadata` defaults to true, and is disabled only by the explicitly test/demo HTTP mock configuration. `SaveTokens = false` and the callback's code-only redirect keep provider tokens out of the SPA URL and browser storage | Making OIDC the default scheme — rejected: it would interfere with existing Bearer API authentication; process-local/manual correlation state — rejected: the handler's cookie-backed nonce/state validation is the supported protocol implementation; globally allowing HTTP metadata — rejected: it would weaken real-provider discovery |
| ADR-16 | OIDC and SAML integration tests use pinned, containerized protocol providers rather than hand-mocked tokens/assertions | Federation failures are commonly in discovery, redirect URI, signature, nonce, certificate, or clock-skew handling; a hand-written fake cannot exercise those protocol boundaries. The OIDC demo/test provider is pinned to `ghcr.io/navikt/mock-oauth2-server:4.0.0`, an actively maintained upstream project with a real Authorization Code Flow and signed discovery/JWKS endpoints. The same Testcontainers pattern is reused for the SAML provider in M21.2. Containers are optional at local test time so ordinary SQLite/unit-test runs remain usable on machines without Docker; CI runs the container-backed tests where Docker is available | Hand-mocking an IdP in C# — rejected: it cannot catch real signature-validation or protocol-state defects; an unpinned `latest` image — rejected: silent upstream changes make federation tests non-reproducible |
| ADR-17 | SAML2 uses `ITfoxtec.Identity.Saml2.MvcCore` 4.20.1 rather than `Sustainsys.Saml2.AspNetCore2` | Sustainsys 2.11.0 was checked and explicitly targets `net8.0` (newer target frameworks are computed compatibility), while ITfoxtec 4.20.1 explicitly targets `net10.0` and provides maintained ASP.NET Core MVC binding and metadata helpers. The service provider loads IdP metadata or a pinned IdP certificate, validates signed assertions through ITfoxtec, maps NameID to `ExternalId`, and reads email from configurable `Saml:EmailAttributeName` (defaulting to the standard `ClaimTypes.Email` URI). Protected RelayState is bound to the AuthnRequest ID to reject unsolicited or replayed responses. | Hand-rolled XML/signature validation — rejected: protocol parsing and cryptography are security-sensitive; Sustainsys 2.11.0 — rejected for lack of an explicit .NET 10 target; hard-coded email claim names — rejected because IdP attribute names vary |
| ADR-18 | SSO enforcement uses a global `AppSetting` plus two per-user flags: `LocalLoginDisabled` and `SsoExempt` | The global setting covers organization-wide SSO-only policy; `LocalLoginDisabled` lets an administrator enforce SSO for one user while the global setting is off; `SsoExempt` is the explicit break-glass path for a trusted administrator. The third flag is necessary because a per-user disable and a global policy are independent controls, while an exemption must remain expressible when the global policy is enabled. Enabling the global policy is rejected unless at least one active System Administrator is SSO-exempt, preventing a settings change from locking out every administrator. | A single per-user/global boolean — rejected: it cannot represent independent per-user enforcement and break-glass access; allowing global enablement with no exempt admin — rejected: recovery would depend on out-of-band database repair |
| ADR-19 | Saved library view filter and sort state is stored as serialized JSON text in portable string columns, not queried as database JSON | View configuration is UI-internal state: the application serializes/deserializes it at the API boundary and applies it to the library query, so no database predicate needs to inspect the configuration. Plain text keeps the same schema and behavior across PostgreSQL, SQL Server, MySQL, and SQLite, while still allowing the client to evolve its filter/sort shape independently. | PostgreSQL `jsonb` — rejected: provider-specific and unnecessary for opaque UI state; normalized view-setting tables — deferred: adds schema and joins for state that is only consumed as a whole |
...
| Recent view's per-user audit-log scan scales with activity volume | A growing audit stream could make the Home Recent query slow or memory-heavy | **Measured (M31.5, 2026-08-18):** 3.38 ms average over 1,000 audit entries for 200 documents and 10 calls after warm-up; `Phase4PerformanceTests` asserts <500 ms average. Details: `doc/Phase4Performance.md` |
| Followed-change hierarchy fan-out scales with nesting and subscriber count | A descendant change could make the notification write path slow or create duplicate notifications | **Measured (M31.5, 2026-08-18):** 71.62 ms average over 20 folder levels and 500 Site/Library subscriptions across 5 calls after warm-up; `Phase4PerformanceTests` asserts <5,000 ms average. Details: `doc/Phase4Performance.md` |
